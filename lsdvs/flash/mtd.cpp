...
int mtd_erase( const char *mtdname, unsigned int start, unsigned int size )
{
	...
	if(fd < 0)
    {
    	SVPrint( "Open(%s) error:%s!", mtdname, STRERROR_ERRNO );
    	return FI_FAILED;
    }
    ...
    {
    	SVPrint( "failed:start+size(%d))>mtd.size(%d) || start(%d)%mtd.erasesize(%d)!=0!\r\n",
    ...
    {
    	SVPrint( "ioctl()error:%s!", STRERROR_ERRNO );
    	Close( fd );
    	return FI_FAILED;
    }
	Close( fd );
...
{    
    ...
    {
    	SVPrint("lseek() error:%s!",STRERROR_ERRNO);
    	Close(fd);
    	return FI_FAILED;
    }    
