...
int WriteFlash( char *mtd, int offset, char *pData, int nLen )
{
	...
	s_FlashMutex.Lock();
	FiPrint( "<WriteFlash: mtd=%s, offset=%X, len=%d>\r\n", mtd, offset, nLen );
    	...
    	while ( nLen > 0 )
        {
        	...
        	if ( memcmp(pWriteData+offset, pData, len) )
            {
                ...
                {        
                	...
                	if ( i == 10 )
                    {
                    	FiPrint( "Erase flash partion %s offset %08X fail\r\n", mtd, start );
                    	nRet = -1;
                    	break;
                    }
            	...
            	if ( j == 10 )
                {
                	FiPrint( "Write flash partion %s offset %08X fail\r\n", mtd, start );
                	nRet = -1;
                	break;
                }
	...
	FiPrint( "<WriteFlash: mtd=%s, offset=%X, len=%d>, nRet(%d)!\r\n", 
...
static int ReadConfigFlash( char* pConfigBuf, int nBufLen )
{
    ...
    {
    	FiPrint( "ReadConfig parameter error !\r\n" );
    	return nRet;
    }
	FiPrint( "ReadConfig, please wait ...\r\n" );
	nRet = ReadFlash( (char *)FLASH_DEVICE_CONFIG, FLASH_OFFSET_CONFIG, pConfigBuf, nBufLen );
	FiPrint( "ReadConfig %s !\r\n", nRet >= 0 ? "OK" : "FAIL" );    
	return nRet >= 0 ? 0 : -1;
}
#endif
...
int ReadConfigBackup( char* pConfigBuf, int nBufLen )
{
	...
	if ( pConfigBuf == NULL || nBufLen < 0 || nBufLen > FLASH_SIZE_CONFIG )
    {
    	FiPrint( "ReadConfig parameter backup error !\r\n" );
    	return nRet;
    }
	FiPrint( "ReadConfig backup, please wait ...\r\n" );
	nRet = ReadFlash( (char *)FLASH_DEVICE_CONFIG, FLASH_OFFSET_CONFIG_BACKUP, pConfigBuf, nBufLen );
	FiPrint( "ReadConfig backup nRet(%d) !\r\n", nRet );    
...
static int WriteConfigFlash( char* pConfigBuf, int nBufLen )
{
	...
	if ( pConfigBuf == NULL || nBufLen < 0 || nBufLen > FLASH_SIZE_CONFIG )
    {
    	FiPrint( "WriteConfig parameter error !\r\n" );
    	return nRet;
    }
	FiPrint( "WriteConfig, please wait ...\r\n" );
	nRet = WriteFlash( (char *)FLASH_DEVICE_CONFIG, FLASH_OFFSET_CONFIG, pConfigBuf, nBufLen );
	FiPrint( "WriteConfig %s !\r\n", nRet >= 0 ? "OK" : "FAIL" );
...
int WriteConfigMsg( char* pConfigBuf, int nBufLen )
{
	...
	if ( pConfigBuf == NULL || nBufLen < 0 || nBufLen > FLASH_SIZE_CONFIG )
    {
    	FiPrint( "WriteConfigMsg parameter error !\r\n" );
    	return nRet;
    }
	FiPrint( "WriteConfigMsg, please wait ...\r\n" );
	nRet = WriteFlashMsg( (char *)FLASH_DEVICE_CONFIG, FLASH_OFFSET_CONFIG, pConfigBuf, nBufLen );
	FiPrint( "WriteConfigMsg %s !\r\n", nRet >= 0 ? "OK" : "FAIL" );
	return nRet >= 0 ? 0 : -1;
}
...
static int ReadLogFlash( char* pLogBuf, int nBufLen )
{
    ...
    {
    	FiPrint( "ReadLog parameter error !\r\n" );
    	return nRet;
    }
	FiPrint( "ReadLog, please wait ...\r\n" );
...
int ReadLogBackup( char* pLogBuf, int nBufLen )
{
	...
	if ( pLogBuf == NULL || nBufLen < 0 || nBufLen > FLASH_SIZE_LOG )
    {
    	FiPrint( "ReadLogBackup parameter error !\r\n" );
    	return nRet;
    }
	FiPrint( "ReadLogBackup, please wait ...\r\n" );
...
static int WriteLogFlash( char* pLogBuf, int nBufLen )
{
	...
	if ( pLogBuf == NULL || nBufLen < 0 || nBufLen > FLASH_SIZE_LOG )
    {
    	FiPrint( "WriteLog parameter error !\r\n" );
    	return nRet;
    }
	FiPrint( "WriteLog, please wait ...\r\n" );
...
int WriteLogBackup( char* pLogBuf, int nBufLen )
{
	...
	if ( pLogBuf == NULL || nBufLen < 0 || nBufLen > FLASH_SIZE_LOG )
    {
    	FiPrint( "WriteLogBackup parameter error !\r\n" );
    	return nRet;
    }
	FiPrint( "WriteLogBackup, please wait ...\r\n" );
...
int WriteLogMsg( char* pLogBuf, int nBufLen )
{
	...
	if ( pLogBuf == NULL || nBufLen < 0 || nBufLen > FLASH_SIZE_LOG )
    {
    	FiPrint( "WriteLogMsg parameter error !\r\n" );
    	return nRet;
    }
	FiPrint( "WriteLogMsg, please wait ...\r\n" );
	nRet = WriteFlashMsg( (char *)FLASH_DEVICE_LOG, FLASH_OFFSET_LOG, pLogBuf, nBufLen );
	FiPrint( "WriteLogMsg %s !\r\n", nRet >= 0 ? "OK" : "FAIL" );
	return nRet >= 0 ? 0 : -1;
}
...
// cramfs 不用全部擦除，jffs2需要，#if 0 封起部分为全部擦除的代码。
#if defined MCU_HI3515
...
int SetUbootEnv( char* pEnv )
{
	...
	if ( nRet == 0 )
    {
    	...
    	if ( crcVal != crcOld )
        {
        	FiPrint( "failed: crc failed with u-boot env!\r\n" );
        	return nRet;
        }
    }    
	else
    {
    	FiPrint( "SetUbootEnv failed: ReadFlash return -1\r\n" );
    	Free( ubootEnv );
    	return nRet;
    }
