...
HI_S32 mpi_comm_vpss_start(HI_S32 s32GrpCnt, SIZE_S *pstSize, HI_S32 s32ChnCnt,VPSS_GRP_ATTR_S *pstVpssGrpAttr)
{
    ...
    if(NULL == pstVpssGrpAttr)
    {
        ...
        stGrpAttr.enDieMode = VPSS_DIE_MODE_AUTO;//VPSS_DIE_MODE_DIE;
    ...
    for(i=0; i<s32GrpCnt; i++)
    {
        ...
        {
            ERRORPRINT("HI_MPI_VPSS_CreateGrp failed with %#x!\n", s32Ret);
            return HI_FAILURE;
        }
        ...
        //tbd....................... vpss param set
        stVpssParam.u32MotionThresh = 8;
        //stVpssParam.u32IeStrength = 255;
        //stVpssParam.u32SfStrength = 255;
        ...
        for(j=0; j<s32ChnCnt; j++)
        {
            ...
            stChnAttr.stFrame.u32Width[VPSS_FRAME_WORK_LEFT]   = 1;
            stChnAttr.stFrame.u32Width[VPSS_FRAME_WORK_RIGHT]  = 1;
            stChnAttr.stFrame.u32Width[VPSS_FRAME_WORK_TOP]    = 1;
            stChnAttr.stFrame.u32Width[VPSS_FRAME_WORK_BOTTOM] = 1;
            
            s32Ret = HI_MPI_VPSS_SetChnAttr(VpssGrp, VpssChn, &stChnAttr);
            if (s32Ret != HI_SUCCESS)
            {
                ERRORPRINT("HI_MPI_VPSS_SetChnAttr failed with %#x\n", s32Ret);
                return HI_FAILURE;
            }
            ...
            if (s32Ret != HI_SUCCESS)
            {
                ERRORPRINT("HI_MPI_VPSS_EnableChn failed with %#x\n", s32Ret);
                return HI_FAILURE;
            }
        }
        ...
        if (s32Ret != HI_SUCCESS)
        {
            ERRORPRINT("HI_MPI_VPSS_StartGrp failed with %#x\n", s32Ret);
            return HI_FAILURE;
        }
