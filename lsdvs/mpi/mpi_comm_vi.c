...
#define DEMO //CX use this macro,nvp do not use
...
*****************************************************************************/
HI_VOID mpi_comm_vi_set_mask(VI_DEV ViDev, VI_DEV_ATTR_S *pstDevAttr)
{
    switch (ViDev % 4)
    {
        case 0:
...
HI_S32 mpi_comm_vi_mode2param(SAMPLE_VI_MODE_E enViMode, SAMPLE_VI_PARAM_S *pstViParam)
{
    switch (enViMode)
    {
...
* function : get vi parameter, according to vi type
*****************************************************************************/
HI_S32 mpi_comm_vi_mode2size(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm, RECT_S *pstCapRect, SIZE_S *pstDestSize)
{
    pstCapRect->s32X = 0;
    pstCapRect->s32Y = 0;
    switch (enViMode)
    {
    	...
    	case SAMPLE_VI_MODE_16_Cif:
        	...
        	break;
        case SAMPLE_VI_MODE_4_D1:
            pstDestSize->u32Width = 720;//D1_WIDTH;//
            pstDestSize->u32Height = (VIDEO_ENCODING_MODE_PAL==enNorm)?576:480;
            pstCapRect->u32Width = 720;//D1_WIDTH;
            pstCapRect->u32Height = (VIDEO_ENCODING_MODE_PAL==enNorm)?576:480;
            break;
        case SAMPLE_VI_MODE_8_2Cif:
            pstDestSize->u32Width = D1_WIDTH / 2;
...
HI_S32 mpi_comm_vi_get_sub_channel_size(VI_CHN ViChn_Sub, VIDEO_NORM_E enNorm, SIZE_S *pstSize)
{
    VI_CHN ViChn;
...
*****************************************************************************/
HI_BOOL mpi_comm_vi_isHD(SAMPLE_VI_MODE_E enViMode)
{
...
HI_S32 SAMPLE_COMM_VI_MemConfig(SAMPLE_VI_MODE_E enViMode)
{
    ...
    for(i=0; i<stViParam.s32ViChnCnt; i++)
    {
        ...
        if (HI_TRUE == mpi_comm_vi_isHD(enViMode))
        {
            ViChn += 16;
...
//sven add for test hi3520d
//#define sven_3520d 0
/*****************************************************************************
* function : star vi dev (cfg vi_dev_attr; set_dev_cfg; enable dev)
*****************************************************************************/
HI_S32 mpi_comm_vi_start_dev(VI_DEV ViDev, SAMPLE_VI_MODE_E enViMode)
{
    HI_S32 s32Ret;
    ...
    switch (enViMode)
    {
    	...
    	case SAMPLE_VI_MODE_1_D1Cif:
            ...
            mpi_comm_vi_set_mask(ViDev,&stViDevAttr);
            break;
        case SAMPLE_VI_MODE_16_D1:
    	case SAMPLE_VI_MODE_8_D1:
        case SAMPLE_VI_MODE_4_D1:
...
#ifdef DEMO
ERRORPRINT("this print is for Data reverse!!!\n");//cx26828  nvp1918c use
    stViDevAttr.bDataRev = HI_TRUE;
#else
ERRORPRINT("this print is not for Data reverse!!!\n");
    stViDevAttr.bDataRev = HI_FALSE;
#endif

//sven add for test hi3520d
#ifdef sven_3520d
memcpy(&stViDevAttr,&DEV_ATTR_7441_INTERLEAVED_720P,sizeof(stViDevAttr));
mpi_comm_vi_set_mask(ViDev,&stViDevAttr);
#endif            
...
*****************************************************************************/
HI_S32 mpi_comm_vi_start_channel(VI_CHN ViChn, RECT_S *pstCapRect, SIZE_S *pstTarSize, 
    SAMPLE_VI_MODE_E enViMode, SAMPLE_VI_CHN_SET_E enViChnSet)
{
    ...
    stChnAttr.bMirror = HI_FALSE;//(VI_CHN_SET_MIRROR == enViChnSet)?HI_TRUE:HI_FALSE;//
    stChnAttr.bFlip = HI_FALSE;//(VI_CHN_SET_FILP == enViChnSet)?HI_TRUE:HI_FALSE;//
...
*****************************************************************************/
HI_S32 mpi_comm_vi_start(SAMPLE_VI_MODE_E enViMode, VIDEO_NORM_E enNorm)
{
    VI_DEV ViDev;
    ...
    s32Ret = mpi_comm_vi_mode2size(enViMode, enNorm, &stCapRect, &stMainTargetSize);
    if (HI_SUCCESS !=s32Ret)
    {
...
#else
    //ERRORPRINT("adc is not inited,take care!!!!!(tbd,cx26808->nvp1914)\n");
#endif
    ...
    for(i=0; i<stViParam.s32ViDevCnt; i++)
    {
        ...
        s32Ret = mpi_comm_vi_start_dev(ViDev, enViMode);
        if (HI_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("mpi_comm_vi_start_dev failed with %#x\n", s32Ret);
            return HI_FAILURE;
        }
    }
    ...
    for(i=0; i<stViParam.s32ViChnCnt; i++)
    {
        ...
        s32Ret = mpi_comm_vi_start_channel(ViChn, &stCapRect, &stMainTargetSize, enViMode, VI_CHN_SET_NORMAL);//VI_CHN_SET_MIRROR);//
        if (HI_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("call SAMPLE_COMM_VI_StarChn %d failed with %#x\n",ViChn, s32Ret);
            return HI_FAILURE;
        } 
        /* HD mode, we will start vi sub-chn */
        if (HI_TRUE == mpi_comm_vi_isHD(enViMode))
        {
            ...
            s32Ret = mpi_comm_vi_get_sub_channel_size(ViChn_Sub, enNorm, &stSubTargetSize);
            if (HI_SUCCESS != s32Ret)
            {
                SAMPLE_PRT("mpi_comm_vi_get_sub_channel_size(%d) failed!\n", ViChn_Sub);
                return HI_FAILURE;
            }
            s32Ret = mpi_comm_vi_start_channel(ViChn_Sub, &stCapRect, &stSubTargetSize,enViMode, VI_CHN_SET_NORMAL);
            if (HI_SUCCESS != s32Ret)
            {
                SAMPLE_PRT("mpi_comm_vi_start_channel (Sub_Chn-%d) failed!\n", ViChn_Sub);
                return HI_FAILURE;
            }
        }
    }
...
HI_S32 SAMPLE_COMM_VI_Stop(SAMPLE_VI_MODE_E enViMode)
{
    ...
    if (HI_SUCCESS !=s32Ret)
    {
        SAMPLE_PRT("mpi_comm_vi_mode2param failed!\n");
        return HI_FAILURE;
    }
...
*****************************************************************************/
HI_S32 mpi_comm_vi_bindvpss(SAMPLE_VI_MODE_E enViMode)
{
    HI_S32 j, s32Ret;
    VPSS_GRP VpssGrp;
    MPP_CHN_S stSrcChn;
