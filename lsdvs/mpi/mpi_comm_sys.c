...
HI_U32 SAMPLE_COMM_SYS_CalcHistVbBlkSize(VIDEO_NORM_E enNorm, PIC_SIZE_E enPicSize, SIZE_S *pstHistBlkSize, HI_U32 u32AlignWidth)
{
    ...
    s32Ret = mpi_comm_sys_get_pic_size(enNorm, enPicSize, &stPicSize);
    if (HI_SUCCESS != s32Ret)
    {
        SVPrint("get picture size[%d] failed!\n", enPicSize);
            return HI_FAILURE;
    }

   SVPrint("stPicSize.u32Width%d,pstHistBlkSize->u32Width%d\n,stPicSize.u32Height%d,pstHistBlkSize->u32Height%d\n",
...
HI_U32 mpi_comm_sys_calcpicVbBlkSize(VIDEO_NORM_E enNorm, PIC_SIZE_E enPicSize, PIXEL_FORMAT_E enPixFmt, HI_U32 u32AlignWidth)
{
    ...
    {
        SVPrint("pixel format[%d] input failed!\n", enPixFmt);
            return HI_FAILURE;
    }

    if (16!=u32AlignWidth && 32!=u32AlignWidth && 64!=u32AlignWidth)
    {
        SVPrint("system align width[%d] input failed!\n",\
               u32AlignWidth);
            return HI_FAILURE;
    }
...
HI_S32 SAMPLE_COMM_SYS_MemConfig(HI_VOID)
{
    ...
    for(i=0;i<32;i++)
    {
        ...
        if (s32Ret)
        {
            SVPrint("HI_MPI_SYS_SetMemConf ERR !\n");
            return HI_FAILURE;
        }
...
HI_S32 mpi_comm_sys_init(VB_CONF_S *pstVbConf)
{
    ...
    if (NULL == pstVbConf)
    {
        ERRORPRINT("input parameter is null, it is invaild!\n");
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        ERRORPRINT("HI_MPI_VB_SetConf failed!\n");
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        ERRORPRINT("HI_MPI_VB_Init failed!\n");
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        ERRORPRINT("HI_MPI_SYS_SetConf failed\n");
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        ERRORPRINT("HI_MPI_SYS_Init failed!\n");
        return HI_FAILURE;
    }
