...
//static int live=0;
static int set_clock=1;   /* non-zero presumably needs root privs */
...
int get_current_freq(void)
{
	...
	txc.modes=0;
	if (adjtimex(&txc) < 0) {
    	perror("adjtimex"); return -1;//exit(1);
    }
	return txc.freq;
#else
	return 0;
...
#endif
//    	if (debug) {
//        	printf("set time to %lu.%.6lu, time(%d)\n", tv_set.tv_sec, tv_set.tv_usec, time(NULL));
//        }
    }
...
void primary_loop(int usd, int num_probes, int interval, int goodness)
{
	...
	to.tv_usec=0;
	for (;;) {
    	switch( NtpAppMaintain() )
        {
    	case NTP_MAINTAIN_NOT_ENABLE: // 如果非使能,这一直在这里死循环
        	continue;
            
    	case NTP_MAINTAIN_RESTART:
        	return;
        }
    	FD_ZERO(&fds);
    	FD_SET(usd,&fds);
...
//	int c;
	...
	int goodness=0;
	char hostBuf[128];
	char *hostname=hostBuf;          /* must be set */
	int ret;                 /* replay mode overrides everything */

	SVPrint( "Thread %s start!\r\n", __FUNCTION__ );
while( NtpAppGetRunFlag() )
{
#if 0
	for (;;) {
	...
	ret = NtpAppGetParam( &cycle_time, hostname );
	SVPrint( "ret(%d), interval(%d), hostname(%s)!\r\n", ret, cycle_time, hostname );
    /* Startup sequence */
	if ((usd=socket(AF_INET,SOCK_DGRAM,IPPROTO_UDP))==-1)
        {perror ("socket");return NULL;} //exit(1);}
